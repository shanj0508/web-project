...
window.dom = {
  create: function create(string) {
    var container = document.createElement("template"); //template中可以存放任何内容
  ...
  remove: function remove(node) {
    //删除节点
    node.parentNode.removeChild(node); //删除节点后，可继续使用这个节点的引用，因此return该node
  ...
  empty: function empty(node) {
    ...
    var array = []; //将node的第一个子节点赋值给x

    var x = node.firstChild; // 当x存在时，执行下面的内容

    while (x) {
      //将node的第一个子元素删除，并将返回值（即被删除的元素）添加到array数组中
      array.push(dom.remove(node.firstChild)); //将node的第一个子节点赋值给x
  ...
  index: function index(node) {
    ...
    var list = dom.children(node.parentNode); //遍历list
...
},{}]},{},["C:/Users/Shanj/AppData/Local/Yarn/Data/global/node_modules/parcel/src/builtins/hmr-runtime.js","dom.js"], null)
